...
function ShellLayoutContent() {
  ...
  useEffect(() => {
    async function loadNavigation() {
      ...
      try {
        const names = Object.values(MFE_NAMES).filter((name) => name !== MFE_NAMES.SETTINGS);
        const results = await Promise.allSettled(names.map((name) => loadMfeNavigation(name)));
        const loadedNavs = results
          .filter((res): res is PromiseFulfilledResult<NavigationItem[]> => res.status === "fulfilled")
          .flatMap((res) => res.value);

        setNavItems([HOME_NAV_ITEM, ...loadedNavs]);
      } finally {
      ...
      <Sidebar>
        ...
        <SidebarHeader className="h-14 border-b border-sidebar-border/50 flex items-center justify-between px-2">
          <NavLink
            ...
            className="flex items-center gap-2.5 w-full focus:outline-none overflow-hidden"
          >
        ...
        <SidebarContent className="px-2 py-2">
          <SidebarMenu>
            ...
            {isNavLoading && navItems.length <= 1 && (
              <div className="flex flex-col gap-1.5 md:gap-1 pt-1">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-[44px] md:h-[32px] rounded-lg md:rounded-md bg-sidebar-accent/40 animate-pulse w-full"
                  />
                ))}
              </div>
            )}
          ...
          <SidebarMenu>
            ...
            <SidebarMenuItem>
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  className="w-[calc(100vw-2.5rem)] max-w-xs md:w-56"
                ...
                >
                  ...
                  <DropdownMenuSub>
                    ...
                    <DropdownMenuSubContent sideOffset={isMobile ? -130 : 4}>
                      <DropdownMenuItem
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        ...
        <header className="flex md:hidden sticky top-0 z-40 h-14 items-center justify-between border-b border-sidebar-border bg-sidebar px-4">
          <div className="flex items-center gap-3">
            <SidebarTrigger className="size-10 [&_svg]:size-5 rounded-xl" />
            <NavLink
              to="/"
              onClick={() => isMobile && setOpenMobile(false)}
              className="flex items-center gap-2 focus:outline-none select-none"
            >
              <span className="text-lg font-black tracking-wider text-brand">FLANER</span>
            </NavLink>
          </div>
        </header>
        ...
        <div className="flex-1 relative flex flex-col">
          ...
          <main className="flex-1 py-3 md:py-8 md:pt-16 px-2.5 sm:px-4 md:px-6 overflow-x-hidden">
            {isNavigating ? (
              <LoadingFallback />
            ) : (
