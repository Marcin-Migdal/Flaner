...
export const MobileNotifications = ({ notificationsOpen, selectedNotificationTab, onChangeTab, authUser }: MobileNotificationsProps) => {
    ...
    const currentNotificationsQuery = selectedNotificationTab === "unread-notification" ? unreadNotificationQuery : allNotificationQuery;

    return (
        <>
            {notificationsOpen !== "closed" &&
                createPortal(
                    <div className={`mobile-notifications ${notificationsOpen}`}>
                        <ul className="mobile-notifications-list">
                            ...
                            <ContentWrapper
                                ...
                                conditions={{
                                    isUninitialized:
                                        currentNotificationsQuery.isUninitialized || currentNotificationsQuery.data?.length === 0,
                                }}
                            >
                                {({ data }) => (
                                    <>
